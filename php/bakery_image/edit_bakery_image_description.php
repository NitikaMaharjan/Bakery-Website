<?php
    ...
    if (isset($_GET['i_id'])) {
        ...
        $sql2 = "SELECT * FROM bakeryimage WHERE i_id = '$i_id'";
        ...
        if ($result->num_rows > 0) {
            ...
            echo "<script>
                    alert('No bakery image found!');
                    window.history.back();
                </script>";
            exit();
        }
    } else {
        echo "Invalid request: bakery image id not provided.";
        exit();
    }
?>
...
<html>
    ...
    <body>
        <div class="page">
            ...
            <div class="contents">
                <div class="three">
                    <h1>Edit Bakery Image Description</h1>
                    <div class="top-right">
                ...
                <div class="bakeryinfo">
                    ...
                    <form action="/LocalBakehouse/php/bakery_image/update_bakery_image_description.php?i_id=<?php echo $i_id; ?>" method="POST">
                        <div class="image">
                            <?php if (!empty($row['i_image'])): ?>
                                <img id="b_image" src="<?php echo htmlspecialchars($row['i_image']); ?>" alt="Bakery image"/>
                            <?php else: ?>
                        ...
                        <div id="form-line">
                            <label for="i_description">Bakery Image Description:</label>
                            <textarea name="i_description" id="i_description" required><?php echo htmlspecialchars($row['i_description']); ?></textarea>
                        </div>

                        <input type="submit" value="Update Bakery Image Description" style="margin-left:730px;"/>
                    </form>
                </div>
            </div>
        </div>
